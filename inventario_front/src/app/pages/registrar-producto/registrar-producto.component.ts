...
export class RegistrarProductoComponent implements OnInit {
  ...
  Registrar(
    nombreControl?: any,
    descripcionControl?: any,
    unidadControl?: any,
    categoriaControl?: any,
    lineaControl?: any,
    precioControl?: any
  ) {
    nombreControl.control.markAsTouched();
    descripcionControl.control.markAsTouched();
    unidadControl.control.markAsTouched();
    categoriaControl.control.markAsTouched();
    lineaControl.control.markAsTouched();
    precioControl.control.markAsTouched();

    const { nombre, descripcion, unidad_medida, categoria_id, linea_id, precio } = this.NewProducto;

    const camposVacios = !nombre || !descripcion || !unidad_medida || !categoria_id || !linea_id;
    const precioValido = precio > 0;

    if (camposVacios || !precioValido) {
